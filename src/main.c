 ...
 *
 *  "include <libavutil/imgutils.h>" provides extra functions for calculating memory for given frames. In simple terms, just learning lol.
 */
 ...
 *  There are one (or multiply) video-streams, audio-streams and if available subtitle-data.
 *
 *
 ...
 *  This can be achieved by using a container-format which supports these features.
 *
 *  A so called MP4 (MPEG-4 Part 14) - format was designed for streaming videos and
 *  having a high compatibility with hardware. This format only supports one video stream
 *  and sometimes multiple audio streams (which is rarely used). Subtitles are also
 *  supported for a limited extend.
 *
 ...
 * Decoder:
 *
 * Video information is basically just an iteration of an array filled with images.
 * Depending on the quality of the image, it could contain a lot of information and
 * thererfore it can have a big filesize. Raw video information can be really big.
 * For that reason there are algorithms for compressing various information.
 * There are many so called "Codecs" which are responsible for the compression.
 * known ones are H.264, VP9 or AV1.
 *
 * A Decoder is something that will take the compress data and ...decodes it. lol
 *
 */
 ...
 *
 * Many video-codecs are using YUV instead of RGB for color and brightness information.
 * YUV can seperate brightness and color information, then these values can be reduced
 * in a better way. So the final video will be reduced in Quality but it won't be really
 * visible. That can reduce the cost of decoding (So I assume that).
 */
...
int main(int argc, char *argv[])
{
    ...
    // iterates through all streams and sets the video StreamIndex to the first video stream found
    ...
    // finds the right decoder of the video. ffmpeg supports extremly many codecs see: "ffmpeg -codecs"
    ...
    // reserves memory for the YUV-images and fills framYUV with the necessary data
